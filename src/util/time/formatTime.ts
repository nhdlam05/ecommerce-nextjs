...
import { TimeInterval } from 'model/calendar';
...
export const getMonthsBetweenDates = (startDate: Moment, endDate: Moment) => {
    ...
    while (
        ...
        totalMonths.push({
            ...
            label: startDate.locale(locale).format('MMMM, YYYY'),
        });

        startDate.add(1, 'month');
    }

    return totalMonths.reverse();
};
...
export function renderFriendlyTimestampString(bookedAtString: string): string {
    ...
    if (dateDiff >= 0 && dateDiff < 1) {
        return `${i18n.t('generic.today')} ${bookedAt
            .locale(locale)
            .format(`[${i18n.t('generic.at')}] HH:mm`)}`;
    }

    return bookedAt
        .locale(locale)
        .format(`dddd, Do MMMM [${i18n.t('generic.at')}] HH:mm`);
}

export function renderFriendlyTimestampWithDurationString(
    bookedAtString: string,
    duration: number
): string {
    const locale: any = i18n.language || UserLang.German;
    ...
    if (dateDiff >= 0 && dateDiff < 1) {
        return `${i18n.t('generic.today')} ${bookedAt
            .locale(locale)
            .format('[,] HH:mm')}-${bookingEndedAtMinute}`;
    }

    return `${bookedAt
        .locale(locale)
        .format('dddd, Do MMMM [,] HH:mm')}-${bookingEndedAtMinute}`;
}

export function renderSimplifiedFriendlyTimestampString(
    bookedAtString: string
): string {
    const locale: any = i18n.language || UserLang.German;
    ...
    if (dateDiff < 0 && dateDiff >= -1) {
        return `${i18n.t('generic.yesterday')}, ${bookedAt
            .locale(locale)
            .format('HH:mm')}`;
    }
...
export function renderFriendlyTimestampStringWithPreviousText(
    bookedAtString: string
): string {
    const locale: any = i18n.language || UserLang.German;
    ...
    const today = moment().startOf('day');
    const dateDiff = bookedAt.diff(today, 'days', true);
    const timeLabel = locale === UserLang.German ? i18n.t('generic.clock') : '';
    ...
    return bookedAt
        .locale(locale)
        .format(
            `[${i18n.t('generic.on')}] dddd, Do MMMM [${i18n.t(
                'generic.at'
            )}] HH:mm [${timeLabel}]`
        );
}
...
export function renderFriendlyWeekDayWithTimestamp(timestamp: string): string {
    ...
    return `${renderFriendlyWeekDay(timestamp)} ${date
        .locale(locale)
        .format(`[${i18n.t('generic.at')}] HH:mm`)}`;
}

export function renderFriendlyDaydiff(date: Moment): string {
    const today = moment().startOf('day');
    const dateDiff = convertFloatToNum(date.diff(today, 'days', false));
    if (dateDiff >= 0 && dateDiff < 2) {
        return dateDiff === 0
            ? i18n.t('generic.today')
            : i18n.t('generic.tomorrow');
    ...
    return `${dateDiff} ${i18n.t('generic.day_plural')}`;
}
...
export const TIME_SLOT_TO_MOMENT_TIME = new Map(
    generateTimeSlotList().map((t) => [t.label, t.value])
);
...
*/
export function validateOverlappingPeriod(
    validatedInterval: TimeInterval,
    intervals: TimeInterval[]
) {
    const startTime = moment(validatedInterval.startTime, 'HH:mm');
    const endTime = moment(validatedInterval.endTime, 'HH:mm');
    for (let i = 0; i < intervals.length; i++) {
        const currentStartTime = moment(intervals[i].startTime, 'HH:mm');
        const currentEndTime = moment(intervals[i].endTime, 'HH:mm');
...
/**
 * Calculates the next target date for a reminder with a provided interval in hours starting now.
 *
 * @param interval time interval in hours
 * @returns Date object containing planned reminder date
 */
...
export function calculateReminderDateTime(
    ...
    if (!reminderTime) {
        ...
        const isTooLate =
            h > ALLOWED_END_HOUR || (h === ALLOWED_END_HOUR && m > 0);
        ...
        scheduledReminder
            .hour(fixedReminderTime.hour())
            .minute(fixedReminderTime.minute());
...
export function renderFriendlyDateInfo(date: Moment): string {
    const locale: any = i18n.language || UserLang.German;
    ...
    if (dateDiff >= 0 && dateDiff < 2) {
        return dateDiff == 0
            ? i18n.t('generic.today').toLowerCase()
            : i18n.t('generic.tomorrow').toLowerCase();
    }
