...
const MethodBadge: React.FC<Props> = ({ item, showExpertised = true, showMoreInfo = true }) => {
  ...
  const showMoreInfoSheet = () => {
    ...
    showModal(<DescriptionModal content={item.detail.description || ''} link={item.detail.link || undefined} />, {
  ...
  return (
    <Badge
      ...
      size='m'
      variant='outlined'
      startSlot={item.expertised && showExpertised ? <AiFillStar fill='#be6b65' /> : <></>}
      onClick={item.detail.description ? showMoreInfoSheet : undefined}
      endSlot={item.detail.description && showMoreInfo ? <RiInformationLine size='18' /> : <></>}
    />
  )
}
