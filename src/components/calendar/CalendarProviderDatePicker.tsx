...
const CalendarProviderDatePicker: React.FC<Props> = ({ openingDay }) => {
    ...
    const [overrideTimeIntervals, setOverrideTimeInterval] = useState<
        OverrideTimeInterval[]
    >([]);
    ...
    const [updateBusyOverrideEvent] = useMutation(
        UPDATE_BUSY_OVERRIDE_EVENT,
        refetchOption
    );
    const [updateNonBusyOverrideEvent] = useMutation(
        UPDATE_OVERRIDE_EVENTS,
        refetchOption
    );
    ...
    async function fetchCurrentDateData(date: Moment) {
        ...
        if (overrideEvents && overrideEvents.length > 0) {
            ...
            if (
                ...
                weeklyAvailabilityData.availabilityEvent
            ) {
                setOverrideTimeInterval(
    ...
    async function handleDelete(id: string) {
        ...
        const nonDeletedEvents = overrideTimeIntervals.filter(
            (e) => e.id !== id
        );
    ...
    function calculateNewTimeInterval() {
        ...
        if (overrideTimeIntervals.length > 0) {
            start =
                overrideTimeIntervals[overrideTimeIntervals.length - 1].endTime;
    ...
    function handleCreateNewTimeInterval() {
        ...
        if (hasBusyOverrideInterval()) {
            ...
            if (validateOverlapping({ startTime: start, endTime: end }, null)) {
                setError(
                    translate('calendar.provider.error.overlappingPeriods')
                );
            } else {
                setError(null);
                setOverrideTimeInterval([
    ...
    function hasBusyOverrideInterval() {
        return (
            overrideTimeIntervals &&
            overrideTimeIntervals.filter(
                (e) => e.eventType === OverrideEventType.Busy
            ).length > 0
        );
    }
    ...
    function renderTimeInterval() {
        ...
        return (
            <>
                {overrideTimeIntervals.map((e) =>
                    renderCalendarTimeInterval(e)
                )}
            </>
        );
    }
                            ...
                            <div>
                                ...
                                <div className="gf gf_h_apart">
                                    <div>
                                        <Title size="s">
                                            {translate(
                                                'calendar.provider.overrides.datePicker.available.title'
                                            )}
                                        </Title>
                                    </div>
