...
const CalendarProviderWeekly: React.FC<Props> = ({ externalCalendarSynced }) => {
  ...
  function renderCalendarProviderWeeklyDay(currentDay: WeekDay) {
    ...
    const currentDateSchedule = weeklyEventData?.availabilityEvents.find(d => d.weekDay === currentDay)

    return (
      <CalendarProviderWeeklyDay key={currentDay} events={currentDateSchedule?.events || []} weekDay={currentDay} />
    )
  }
    ...
    <div className='CalendarProviderWeekly'>
      <div className='CalendarProviderWeekly--header'>
        <Section spacingBottom='s'>
          <Title size='m'>{translate('calendar.provider.weekly.title')}</Title>
          <Text size='s'>{translate('calendar.provider.weekly.subtitle')}</Text>

          <Section spacingTop='xs'>
            {externalCalendarSynced ? (
              <Box display='flex' alignItems='center'>
                <Box sx={{ mr: 1 }}>
                  <Icon icon={<BsCalendar3 />} theme='success' size='xxs' />
                </Box>
              ...
              <Box display='flex' alignItems='center'>
                <Box sx={{ mr: 1 }}>
                  <Icon icon={<BsCalendar3 />} theme='action' size='xxs' />
                </Box>

                <Title size='s' highlighted noMargin>
                  <Link to='/settings/integration'>{translate('generic.sync.calendar')}</Link>
                </Title>
              </Box>
            )}
          </Section>
      ...
      {loading && <Loader />}
      {!loading && WEEK_DAYS.map(d => renderCalendarProviderWeeklyDay(d))}
    </div>
  )
}
