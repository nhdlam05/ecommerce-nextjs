...
const CalendarProviderOverrideItem: React.FC<Props> = ({
  ...
  function isUnavailableDate() {
    return !events || events.length === 0 || events.filter(e => e.type === OverrideEventType.Busy).length > 0
  }

  return (
    <div className='CalendarProviderOverrideItem'>
      <button className='CalendarProviderOverrideItem--dates' disabled={disabled} onClick={handleItemClick}>
        <div>
