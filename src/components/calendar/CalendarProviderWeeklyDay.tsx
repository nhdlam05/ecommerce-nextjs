...
const CalendarProviderWeeklyDay: React.FC<Props> = ({ weekDay, events }) => {
    ...
    const [deleteAvailabilityEvent] = useMutation(
        DELETE_AVAILABILITY_EVENT,
        refetchOption
    );
    const [updateAvailabilityEvent] = useMutation(
        UPDATE_AVAILABILITY_EVENT,
        refetchOption
    );
    const [createAvailabilityEvent] = useMutation(
        CREATE_AVAILABILITY_EVENT,
        refetchOption
    );
    ...
    function shouldDisableAddButton() {
        ...
        return (
            ...
            moment(
                currentEvents[currentEvents.length - 1].endTime,
                'HH:mm'
        ...
        <div className="CalendarProviderWeeklyDay">
            <div className="CalendarProviderWeeklyDay--inner">
                ...
                <div className="gf gf_h_apart">
                    <div>
                        <Title size="m">
                            {translate(WEEK_DAY_TRANSLATION_KEY[weekDay])}
                        </Title>
                    </div>
