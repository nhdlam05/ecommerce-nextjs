...
const CalendarNextAvailability = () => {
  ...
  const { data: myProviderProfileRes } = useQuery(GET_PROVIDER_AVAILABILITY_INFO)
  const { translate } = useTranslationWithContext()

  const getDataByNextAvailabilitySlot = (nextAvailabilitySlot: AvailabilityInterval): AvailabilityData => {
    const { startDateTime } = nextAvailabilitySlot
    const today = moment().startOf('day')
    const dateDiff = convertFloatToNum(moment(startDateTime).diff(today, 'days', true))
    ...
    switch (true) {
      case dateDiff >= 0 && dateDiff <= 8:
        return {
          theme: 'success',
          icon: '🌿',
          title: <Typography variant='body1'>{renderFriendlyTimestampString(startDateTime)}</Typography>,
          subtitle: (
  ...
  const getData = (
    ...
    if (!availableForNewBooking) {
      return {
        theme: 'purple',
        icon: '🎋',
        title: translate('calendar.next.availability.waiting.list.title'),
        subtitle: translate('calendar.next.availability.waiting.list.subtitle')
      }
    }
