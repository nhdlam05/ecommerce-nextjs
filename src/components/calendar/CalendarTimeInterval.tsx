...
import { AVAILABLE_TIME_SLOTS, TIME_SLOT_TO_MOMENT_TIME } from 'util/time/formatTime'
...
type Props = {
  ...
  handleUpdate: (id: string, startTime: string, endTime: string) => Promise<void>
} & SelectFieldData

const SELECTABLE_AVAILABLE_TIMESLOT = ['', ...AVAILABLE_TIME_SLOTS]

const CalendarTimeInterval: React.FC<Props> = ({ id, startTime, endTime, handleDelete, handleUpdate }) => {
  const {
    control,
    formState: { errors }
  ...
  async function handleStartTimeFieldChange(target: any) {
    ...
    if (validateDurationAndSetError(target.value, currentEndTime)) {
      await handleUpdateTimeInterval(target.value, currentEndTime as string)
    }
  }
  ...
  async function handleEndTimeTimeFieldChange(target: any) {
    ...
    if (validateDurationAndSetError(currentStartTime, target.value)) {
      await handleUpdateTimeInterval(currentStartTime as string, target.value)
    }
  }

  function validateDurationAndSetError(start: string | null, end: string | null): boolean {
    if (!start || !end) {
...
return false
    ...
    if (
      ...
      TIME_SLOT_TO_MOMENT_TIME.get(end)?.isSameOrBefore(TIME_SLOT_TO_MOMENT_TIME.get(start))
    ) {
      setError(translate('calendar.provider.error.endTimeMustBeAfterStart'))
            ...
            render={({ field: { onChange, value } }) => (
              <SelectInput
                ...
                onChange={async (event: any) => {
                  onChange(event)
                  await handleStartTimeFieldChange(event.target)
                }}
          ...
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <SelectInput
                ...
                onChange={async (event: any) => {
                  onChange(event)
                  await handleEndTimeTimeFieldChange(event.target)
                }}
